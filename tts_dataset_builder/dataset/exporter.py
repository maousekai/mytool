...
def inspect_zip(zip_path: str) -> Dict[str, object]:
    """Return basic integrity information and raise for a corrupt/non-ZIP file."""
    ...
    if magic != b"PK\x03\x04":
        raise RuntimeError(
            f"Invalid ZIP signature: expected 50 4B 03 04, got {magic.hex(' ')}"
        )
...
def export_dataset_to_zip(dataset_dir: str = "output_dataset", output_zip_path: Optional[str] = None) -> str:
    ...
    if info["wav_count"] != len(wav_files):
        raise RuntimeError(
            f"ZIP WAV count mismatch: expected {len(wav_files)}, found {info['wav_count']}"
        )
    if "metadata.csv" not in info["names"]:
        raise RuntimeError("ZIP validation failed: metadata.csv is missing")

    logger.info(
        "Dataset ZIP PASS | path=%s | size=%.2f MB | wav_files=%d",
        target_zip, int(info["size_bytes"]) / (1024 * 1024), len(wav_files),
    )
    return target_zip


def export_source_package(source_dir: str, output_zip_path: str) -> str:
    """Create a fresh standalone package instead of serving a stale prebuilt ZIP."""
    ...
    logger.info(
        "Source package ZIP PASS | path=%s | size=%.2f MB | files=%d",
        target_zip, int(info["size_bytes"]) / (1024 * 1024), int(info["file_count"]),
    )
    return target_zip
